...
def resolve_symbol(exchange: ccxt.Exchange, sym: str) -> str:
    ...
    if sym.endswith("/USDT") and f"{sym}:USDT" in markets: return f"{sym}:USDT"
...
def post_discord_outcome(s: dict, kind: str, price: float, rr: float, ts_iso: str) -> None:
    ...
    payload = {"username": "StructureBot","embeds": [{"title": title,"color": color,"fields": fields,"footer": {"text": f"exited {ts_iso}"}}]}
    try:
...
if __name__ == "__main__":
    ...
    tf = cfg.get("outcomes", {}).get("tf_for_check", "1m")
    max_age_h = int(cfg.get("outcomes", {}).get("max_age_hours", 8))
    miss_after_h = int(cfg.get("outcomes", {}).get("miss_after_hours", 2))
    poll_s = int(cfg.get("outcomes", {}).get("poll_seconds", 60))
    post_outcomes = bool(cfg.get("outcomes", {}).get("post_to_discord", False))

    print(f"[OUTCOME] start tf={tf}, max_age={max_age_h}h, miss_after={miss_after_h}h | table={db.table}")
        ...
        for s in rows:
            ...
            if s.get("entered_at"):
                ...
                for ts, o, h, l, c, v in ohlcv:
                    if l <= entry <= h:
                        entered_ms = ts
                        db.update_signal(s["id"], {"entered_at": datetime.fromtimestamp(ts / 1000, timezone.utc).isoformat(),"outcome": "open","last_checked": datetime.now(timezone.utc).isoformat(),})
                        print(f"[OUTCOME] {s['id']} entered"); break
                if not entered_ms:
                    db.update_signal(s["id"], {"last_checked": datetime.now(timezone.utc).isoformat()}); continue

            start_idx = next((i for i, row in enumerate(ohlcv) if row[0] >= entered_ms), 0)

            hit = None
            for i in range(start_idx, len(ohlcv)):
                ts, o, h, l, c, v = ohlcv[i]
                if side == "LONG":
                    if l <= stop: hit = ("sl", stop, i); break
                    if h >= tp1: hit = ("tp", tp1, i); break
                else:
                    if h >= stop: hit = ("sl", stop, i); break
                    if l <= tp1: hit = ("tp", tp1, i); break
            ...
            if hit:
                ...
                db.update_signal(s["id"], {"outcome": kind,"exited_at": ex_time,"exit_price": float(price),"rr_achieved": float(rr),"last_checked": datetime.now(timezone.utc).isoformat(),})
                print(f"[OUTCOME] {s['id']} {symbol} -> {kind} @ {price} (RR={rr:.2f})")
                if post_outcomes: post_discord_outcome(s, kind, float(price), float(rr), ex_time)
            else:
                if age_h >= max_age_h:
