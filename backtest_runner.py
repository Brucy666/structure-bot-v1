#!/usr/bin/env python3
"""
StructureBot v2 — Research Backtester (clean)

- BOS/SFP detection identical to live (hard regime gate + ATR percentile + allowed types/hours)
- Dynamic entry ("auto") and ATR-padded stops
- Outcome simulation (limit entry; SL priority on same bar)
- Supabase inserts (is_backtest=true, RUN_ID tag, on_conflict=dedupe_key)
- CSV output per (symbol, timeframe)
...
ENV:
  ...
  RUN_ID="v2_<tag>"            (optional; auto if missing)
"""
...
def fetch_ohlcv_cached(symbol: str, timeframe: str, since_ms: int, until_ms: int,
    ...
    cache_file = pathlib.Path(cache_dir) / f"{exchange_name}_{symbol.replace('/', '-').replace(':','-')}_{timeframe}.parquet"
    cached = None
...
def detect_impulse_and_zone(ohlcv: np.ndarray, cfg: dict) -> Optional[Zone]:
    ...
    for i in range(len(ohlcv) - 2, cfg["impulse"]["atr_len"], -1):
        ...
        if bod >= cfg["impulse"]["body_min"] and rng >= cfg["impulse"]["atr_mult"] * cur_atr:
            ...
            if run_dir == 0 or d == run_dir:
                ...
                break
            if run_len >= cfg["impulse"]["min_consecutive"]:
                break
        elif run_len > 0:
            break

    if end_idx is None or run_len < cfg["impulse"]["min_consecutive"]:
        return None
    ...
    return Zone("bullish" if run_dir == 1 else "bearish",
                float(max(zone_top, zone_bottom)),
                float(min(zone_top, zone_bottom)),
...
def build_plan(ohlcv: np.ndarray, zone: Zone, sig: Dict, cfg: dict) -> Dict:
    ...
    if sig["direction"] == "bullish":
        ...
        stop  = float(zone.bottom - pad * cur_atr)
        risk  = max(abs(entry - stop), 1e-9)
        tp1   = entry + rr * risk
    else:
        entry = float(zone.top - frac * width)
        stop  = float(zone.top + pad * cur_atr)
...
def score_signal(zone: Zone, sig: Dict, plan: Dict, cfg: dict,
    ...
    clean = max(0.0, 1.0 - abs(plan["entry"] - sig["level"]) / max(plan["atr"], 1e-9))
...
def run_backtest(cfg: dict, args):
    ...
    table = (research.get("signals_table")
    ...
    supa = Supa(sb_url, sb_key, table) if (push_db and sb_url and sb_key) else None
    ...
    for sym in symbols:
        for tf in timeframes:
            ...
            for i in range(window, len(df) - 1):
                ...
                ok, _why = allowed_by_filters(cfg, sym, tf, sig["type"], when_utc, atr_ser_all[:i+1])
                if not ok:
                    continue
                ...
                if cfg["filters"].get("use_htf_bias", False):
                    try:
                        ...
                        htf = ex.fetch_ohlcv(rsym, cfg["filters"]["htf_timeframe"], limit=200)
