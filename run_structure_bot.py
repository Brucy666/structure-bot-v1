...
def make_payload(symbol, timeframe, cfg, zone, sig, candles, plan):
    ...
    return {
        ...
        "embeds": [{
            ...
            "footer":{"text":"BOS/SFP from last impulse wick→body zone • Retest entries"}
        }]
    }
...
if __name__ == "__main__":
    ...
    POST_DEBUG = bool(dbg.get("post_debug_to_discord", False))  # stays False in prod
            ...
            for tf in valid_tfs:
                try:
                    ...
                    if not zone:
                        ...
                        if POST_DEBUG:
                            notify.post({"username":"StructureBot (debug)","embeds":[{"title":f"Watching {symbol} {tf}","description":"No valid impulse/zone yet."}]})
                        continue
                    ...
                    for sig in (bos, sfp):
                        ...
                        print(f"[ALERT] {symbol} {tf} — {sig['type']} {sig['direction']} @ {sig['level']:.2f} → {plan['side']} (entry {plan['entry']:.2f})")
                        fired = True

                    if not fired and POST_DEBUG:
                        notify.post({"username":"StructureBot (debug)","embeds":[{"title":f"Watching {symbol} {tf}","description":f"{zone.kind} zone {zone.bottom:.2f} → {zone.top:.2f}\nNo BOS/SFP yet."}]})
