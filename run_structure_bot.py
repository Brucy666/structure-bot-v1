...
def detect_impulse_and_zone(ohlcv: np.ndarray, cfg: dict) -> Optional[Zone]:
    ...
    return Zone(kind=kind,
                top=float(max(zone_top, zone_bottom)),
                bottom=float(min(zone_top, zone_bottom)),
...
def allowed_by_filters(cfg: dict, symbol: str, tf: str, sig_type: str,
                       now_utc: datetime, atr_series: np.ndarray) -> Tuple[bool, str]:
    ...
    if cfg.get("startup_backfill", {}).get("enabled", True):
        ...
        for sym in symbols:
            for tf in tfs:
                ...
                ok, _why = allowed_by_filters(cfg, sym, tf, sig["type"], datetime.now(timezone.utc), atr_ser)
                if not ok:
                    continue
                ...
                if any(k == key and (now - t).total_seconds() < dedupe_minutes * 60 for k, t in seen):
                    continue
                seen.append((key, now))
